...
{#if message.from === "assistant"}
	...
	>
		...
		/>
		<div
			class="relative flex min-w-[60px] flex-col gap-2 break-words rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50/50 to-white px-5 py-3.5 text-slate-700 prose-pre:my-2 dark:border-gray-800 dark:from-gray-800/80 dark:text-gray-300"
		>
	...
	>
		<div class="flex w-full flex-col gap-2">
			...
			<div class="flex w-full flex-row flex-nowrap">
				{#if !editMode}
					<p
						class="disabled w-full appearance-none whitespace-break-spaces text-wrap break-words bg-inherit px-5 py-3.5 text-slate-600 dark:text-gray-400"
					>
